...
const JOB_TITLE_NOISE = [
  ...
  // 「新卒採用サイトの運用」「新卒採用ページの制作」＝ その仕事を中途で募集している求人
  /新卒(?:採用)?(?:サイト|ページ|情報)(?:の)?(?:運用|制作|企画|リニューアル|改修|更新|設計|開発)/g,
  /採用担当[（(][^）)]{0,12}新卒[^）)]{0,12}[）)]/g,
...
// 「会社説明会」は入れない。一般的な紹介文（「会社説明会だけではイメージしにくい」）にも、
// 中途求人の職務内容（「学内セミナーや会社説明会の開催機会を獲得し」）にも出るため、
// 実測で誤検出2件の原因になった（テクノツリー／ウィルオブ・ワーク）。
const CYCLE_WORDS = ['卒業見込', 'プレエントリー', '新卒エントリー', '新卒採用エントリー',
  '新卒マイページ', '新卒採用サイト', '新卒採用情報', '新卒採用ページ', '新卒募集要項',
  '新卒採用選考', '新卒応募', '新卒採用フロー', '新卒選考'];
