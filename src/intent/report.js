...
function buildReport(rows, stats, opts = {}) {
  ...
  for (let i = 0; i < top.length; i++) {
    ...
    // 採用構成は「なぜこの会社を選んだか」そのものなので、取れていれば必ず出す。
    if (r['採用構成'] && r['採用構成'] !== '不明') {
      L.push(`- 採用構成: ${r['採用構成']}（新卒 ${r['新卒規模'] || '—'}／中途求人 ${r['中途求人件数'] || '未取得'}件）`
        + (r['採用構成根拠'] ? `／${String(r['採用構成根拠']).slice(0, 140)}` : ''));
    }
    // 昨年度の結果と資金面は、架電の入り口そのものなので上位社では必ず出す。
    if (r['昨年度入社数'] || r['昨年度定着率']) {
      L.push(`- 昨年度の採用: ${r['昨年度入社数'] || '—'}／定着率 ${r['昨年度定着率'] || '—'}`
        + (r['定着率(3年)'] ? `（3年: ${r['定着率(3年)']}）` : ''));
    }
    if (r['予算状態'] && r['予算状態'] !== '未判定') {
      L.push(`- 資金: ${r['予算状態']}（係数${r['予算係数'] || 1}）`
        + (r['検討時期'] ? `／検討時期 ${r['検討時期']}` : '')
        + (r['資金リスク'] ? `／${String(r['資金リスク']).slice(0, 120)}` : ''));
    }
    L.push(`- 次の対応: ${r.推奨アクション}／${r.提案ルート}／要確認:${r.要確認項目 || 'なし'}`);
    L.push(`- 根拠: ${r['根拠']}`);
    if (r.根拠URL一覧) L.push(`- 根拠URL: ${r.根拠URL一覧}`);
    if (r['推奨トーク']) L.push(`- トーク: ${r['推奨トーク']}`);
    if (r['予算トーク']) L.push(`- 予算トーク: ${r['予算トーク']}`);
    L.push('');
  }
  return L.join('\n');
}
