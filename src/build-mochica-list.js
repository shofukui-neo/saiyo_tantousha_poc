...
const { scoreMochica, FUNNEL_TH } = require('./mochica-fit');
...
const INTENT_BORROW = ['新卒フラグ', '新卒出稿', '現在求人掲載中', '掲載媒体', '掲載媒体数', '出稿媒体数',
  ...
  // ③ 採用ファネル（エントリー数×採用人数×歩留まり＝MOCHICAの刺さり）
  'エントリー数', 'エントリー人数', 'プレエントリー数', '応募者数', '応募数',
  '採用人数', '採用数', '内定者数', '歩留まり', '歩留り', '歩留まり率', '内定承諾率', '選考通過率'];
...
async function main() {
  ...
  const SCORE_COLS = ['アポ期待度', '優先度', '確信度', 'なぜ今なぜこの企業',
    'INT', 'FUNNEL', 'SIZE', 'REACH', 'TIM', 'TRUST', '採点根拠'];
  ...
  for (const { rec, s } of scored) {
    ...
    outRecords.push(Object.assign({}, rec, {
      ...
      'INT': s.dims.intent, 'FUNNEL': s.dims.funnel, 'SIZE': s.dims.size, 'REACH': s.dims.reach,
      'TIM': s.dims.timing, 'TRUST': s.dims.trust,
      '採点根拠': s.reasons.join(' / '),
    }));
  }
  ...
  console.log('\n' + L);
  console.log('  MOCHICA営業リスト — アポ取得期待値モデル v2（採用ファネル軸を追加）');
  console.log(L);
  ...
  console.log(`  採用ファネル目安: エントリー${FUNNEL_TH.entry}人以上 × 採用${FUNNEL_TH.hire}人以上 × 歩留まり${FUNNEL_TH.yieldMax}%以下`);
  console.log(L);
  ...
  console.log(`    ② 採用ファネル : ${avg(scored, 'funnel')}  ${bar(avg(scored, 'funnel'))}`);
  ...
  console.log(`    採用ファネル痛みを確認    : ${agg(headN, x => x.s.flags.funnelFit)}%   ← 「エントリー/採用/歩留まりの2つ以上が目安内」`);
  console.log(`    大型採用(E100+×採用10+)  : ${agg(headN, x => x.s.flags.bigFunnel)}%   ← 「大量母集団×大型枠＝最刺さり」`);
  ...
  console.log(L);
  console.log(`  ◆ 今週架電すべき上位${Math.min(top, scored.length)}社（期待度｜INT/FUNNEL/SIZE/REACH/TIM｜確信度）`);
  scored.slice(0, top).forEach((x, i) => {
    const d = x.s.dims;
    console.log(`   ${String(i + 1).padStart(2)}. ${String(x.s.total).padStart(3)}｜${String(d.intent).padStart(3)}/${String(d.funnel).padStart(3)}/${String(d.size).padStart(3)}/${String(d.reach).padStart(3)}/${String(d.timing).padStart(3)}｜確信${String(x.s.confidence).padStart(3)}  ${x.rec['企業名'] || ''}`);
    console.log(`       └ ${x.s.why}`);
  });
  console.log(L);
  ...
  console.log(`     ${callableTop}% は今日そのまま架電できる。汎用属性ではなく“新卒インテント×採用ファネル(E${FUNNEL_TH.entry}+/採用${FUNNEL_TH.hire}+/歩留${FUNNEL_TH.yieldMax}%↓)×規模×到達性×28卒設計期”で選定。\n`);
}
