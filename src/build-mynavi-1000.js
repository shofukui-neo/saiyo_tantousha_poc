...
const HEADERS = ['企業名', 'corpID', 'マイナビ掲載', '採用担当者名', '担当者確度', 'パターン', '担当者根拠',
  '役職', '部署', 'メール', '電話番号', '従業員数', '募集職種', '募集職種数', '採用予定人数', '卒年', '採用ページURL', '取得日'];
// 「募集職種数」は scrape-mynavi が既に持っていた値（extractIntentSignals）。ここまで出力していなかったため
// signal-store が求人数の時系列を作れず、シグナル最上位の「求人急増」が原理的に出せなかったので出力に加える。
    ...
    for (const kw of keywords) {
      ...
      for (const f of fresh) {
        ...
        const row = { 企業名: r.企業名 || f.name, corpID: f.id, マイナビ掲載: r.マイナビ掲載 || '○',
          ...
          電話番号: r.電話番号 || '', 従業員数: r.従業員数 || '', 募集職種: r.募集職種 || '',
          募集職種数: r.募集職種数 || '', 採用予定人数: r.採用予定人数 || '',
