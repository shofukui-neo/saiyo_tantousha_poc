...
function matchRow(name, corp, ctx) {
  ...
  if (ctx.cust) {
    ...
    else if (nm && ctx.cust.byName.has(nm)) hits.push({ source: '既存顧客', conf: '推定', key: `社名:${name}` });
  }
  // ③ SFリード（法人番号→社名）
  if (ctx.sf) {
    if (cn && ctx.sf.byCorp.has(cn)) hits.push({ source: 'SFリード', conf: '確実', key: `法人番号:${cn}`, sf: ctx.sf.byCorp.get(cn) });
    else if (nm && ctx.sf.byName.has(nm)) hits.push({ source: 'SFリード', conf: '推定', key: `社名:${name}`, sf: ctx.sf.byName.get(nm) });
  }

  // 代表理由の優先順位: 禁止 > 既存顧客 > SFリード
  const order = { '禁止': 0, '既存顧客': 1, 'SFリード': 2 };
...
function main() {
  ...
  // 照合ソースを構築（無いものはスキップ）
  const ctx = { ng: null, cust: null, sf: null };
  const skipped = [];
  ...
  const clean = [];
  const excluded = [];
  const cnt = { 禁止: 0, 既存顧客: 0, SFリード: 0 };   // ソース別（重複カウントあり）
  let withCorpNum = 0;
