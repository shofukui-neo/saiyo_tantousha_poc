...
>
> **改訂 2026-08-17（asumo 側の検証を受けて）**: 本書は初版。asumo 側の確認事項28項目には
> [asumo-list-migration-answers.md](asumo-list-migration-answers.md) で実データを取り直して回答した。
> **回答の結果、本書の以下が変わる**——順序（P0 を新設し P1 より前へ）／P1 のスコープ（tier5 は実装しない）／
> P4 の位置づけ（新モジュール → 既存 `call-memo-infer.ts` への規則追加）／P5 の媒体数（110 → **106**）。
> 確定版は回答書 §G2 の表を正とする。
...
| **G4** | **母集団を増やす入口が gBiz 1本** | `prospect-discover` は gBizINFO のみ（`lib/prospect/search-api.ts` 159行はどこからも呼ばれていないデッドコード）。**ただし 08-17 に ICP適合リスト（`source="icp"`）が入り、トークン待ちの間も既存1,182社から当たり先は決められる**——「母集団が増えない」だけで「架電できない」ではない | gBiz は「**新卒採用しているか**」を持たない → intent（重み0.30）が観測頼みの弱いティアに固定される。媒体入口なら「新卒募集中」が構造的に保証される | poc の `media-catalog.json` **106媒体**を probe 較正済み。**企業リンク10本以上を静的に出すのは27媒体**、実際に氏名まで取れたのは地方特化33媒体中**1媒体のみ**（にいがた型） |
...
- **`fuzzy`（長音ゆれ / tier5）は実装しない**（改訂）。本日 28,137件×マスタ56,165社で再実行した結果、
  この tier のヒットは **0件**だった（内訳: 社名83 / 表記ゆれ30 / 長音ゆれ0）。別法人衝突の実例
  （ファミリー/ファミリ）を抱える唯一の段なので、実害ゼロの機能のためにリスクを取る理由が無い。
  必要になったら足せるよう、キー生成関数だけは移植して**索引に載せない**形にする。
- **新しい tier には新しいガードが要る**。K2 の `namesLookRelated` は「ドメイン一致時に社名が関連するか」
  を見るガードなので、社名だけの索引（`byLoose`/`byCore`）には効かない。§A3 の3種
  （連合会/中央会・HD非除去・1文字キー禁止）が poc 側で用意してあるガードの正体で、これを一緒に移す。
...
**★ 調査済み（2026-08-17）: 重複していた。** poc の `isUntouchedTemplate`（ヒアリング様式の選択肢
7項目中6つ以上が残っていたら未編集テンプレ＝課題として数えない）は、asumo `call-memo-infer.ts:187-194`
が実害として記録している現象と**同一**。よって **P4 は新モジュールではなく既存への規則追加**とする。
poc から持ち込む価値があるのは、テンプレガード以外の3層——**充足規則8本**（「母集団は足りている」で
打ち消す）・**中途ガード**（痛みが中途/派遣に帰属する文を落とす）・**自社プロダクトガード**
（`MOCHICA|モチカ|弊社|当社` の8文字以内に「母集団」＝顧客のニーズではない）——と、強/中の2段階。
`FREE_FIELDS`（走査対象の列）は引数で注入できる形に直してから移すこと。
...
**やること**: `data/media-catalog.json`（**106媒体**・全件 probe 較正済み）を asumo に持ち込み、
**静的に企業の外部リンクを出す媒体だけ**（＝企業リンク10本以上を保持する **27媒体**）を
企業母集団の入口として使う（poc [media-crawl.js](../src/media-crawl.js) の `crawlMediaForCompanies` 相当）。

> **着手前に測ること**: poc の probe は手元PCから、asumo は Fly の DC-IP から叩く。
> DDG/Bing では IP で挙動が変わる実例があった（`search.js`）。媒体側は未検証なので、
> **Fly から数媒体を probe して poc の結果と突き合わせてから**本走する。
...
**★ 2026-08-17 改訂**。poc 実害の主因を調べ直した結果（回答書 §B3）、80〜90%の被りを作ったのは
**キーではなく経路**——ビルダーが3マスタと1度も突合していなかったことだった。落ちた40社中34社は
strict キーですら当たる企業で、tier の追加が効いたのは2社程度。asumo で同型なのは
`/list-import` が除外台帳を見ていない点なので、**そこを P0 として P1 より前に置く**。

| 順 | フェーズ | 規模 | 前提 | 効果 |
|---|---|---|---|---|
| **0** | **P0 `/list-import` に除外台帳ゲート**（`checkProspectExclusion` を通し、落とした行は `import_rows` に理由付きで残す） | **0.5人日** | なし | **poc の実害の主因と同型**。最小の変更で最大の穴が閉まる |
| 1 | **P1 表記ゆれ突合**（法人番号・農協・tier4まで。**tier5 は載せない**） | 1〜1.5人日 | なし | 全経路に効く。除外の取りこぼしを塞ぐ |
| 2 | **P2 監査ゲート** | 1人日 | P1 | P1 の効き方を目で確認できる。以後の全変更の安全網 |
| 3 | **P3 feeder（担当者名）**（`feeder:recruitpage` を先に。`feeder:mynavi` は規約確認後） | 2〜3人日 | P0〜P2 | 名指し架電が成立する＝アポ率に直結 |
| 4 | **P4 再アプローチ層**（既存 `call-memo-infer.ts` への**規則追加**） | 1.5〜2人日 | — | 発掘コスト0の温かい層。母数の3.2%（母集団課題）／6.4%（番号不備） |
| 5 | P5 媒体入口（実質**27媒体**） | 3〜5人日 | P1〜P3 | intent 最強ティアの母集団 |
| 6 | P6 BALESエクスポート | 0.5人日 | 必要時 | 併用期間の橋渡し |

**合計 9.5〜13人日**（P5 まで）。**P0+P1+P2+P3 の 5〜7人日で「呼べる名指しリストが正しく作れる」状態**に到達する。
